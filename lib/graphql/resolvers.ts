...
export const resolvers = {
  Query: {
    ...
    getSectionsByCourse: async (
      ...
      await Section.find({ course: courseId })
        .sort({ order: 1 })
        .populate("lessons"),
    ...
    getInstructorCourses: async (_: unknown, __: unknown, context: any) => {
  ...
  Mutation: {
    ...
    createAdmin: async (_: unknown, { userId }: { userId: string }) => {
      const admin = await Admin.create({ user: userId });
    ...
    createCourse: async (
      ...
      const course = new Course({
        ...args,
        instructor: context.user.id, // ← from NextAuth session
      });
    ...
    updateProgress: async (
      ...
      const existing = progress.watchTime.find(
        (w: { lesson: unknown; seconds: number }) =>
          String(w.lesson) === lessonId
      );
      if (existing) {
